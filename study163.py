...
一个用于爬取网易云课堂 MOOC 课程资源的爬虫，仅适用于 mooc.study.163.com 域下的课程。
和中国大学MOOC不同的是，只有加入了课程，才能获取到课程资源，所以必须登录并且参加了课程。

可以获得视频、富文本、附件以及字幕等资源

使用方法：向 start 函数传入课程 URL，例如：
http://www.icourse163.org/course/HUST-1001942004
或
http://www.icourse163.org/course/HUST-1001942004?tid=1002047009

第一个链接可以获取最近一次开课的内容（包括即将开课和正在进行的课程）
第一个链接可以获得第 n 次开课的内容

第二个参数指定下载的目录，如果不指定默认为该 Python 程序的位置

第三个参数指定是否需要下载 PDF 文件

详细使用方法见：https://mooc.xoy.io/

如果遇到 bug 或有什么好的建议，可以在 GitHub 中发布 issue 或者通过邮箱 master@foair.com 联系我
"""
...
def login(account, password):
  ...
  login_addr = 'https://study.163.com/passport/cellphone/login.htm'
  data = {'mobile': '', 'password': '', 'saveFlag': 'true', 'returnUrl': 'aHR0cDovL3N0dWR5LjE2My5jb20vP2Zyb209c3R1ZHk='}
  result = CONNECTION.post(login_addr, data=data).json()
  if result['code'] == 23:
    print('用户名或密码错误！')
  else:
    print('登录成功')
    print(result)
    with open('cookie.json', 'w') as cookiefile:
      json.dump(CONNECTION.cookies.get_dict(), cookiefile)
...
def get_summary(url):
  ...
  # 文件夹名称
  dir_name = course_name + ' - ' + institution
  print(dir_name)
...
def get_announce(term_id):
  ...
  with open(os.path.join(BASE_DIR, 'Announcements.html'), 'w', encoding='utf-8') as announce_file:
    for announcement in announcements:
      # 公告内容
      announce_content = announcement[0].encode('utf-8').decode('unicode-escape')
      # 公告标题
      announce_title = announcement[1].encode('utf-8').decode('unicode-escape')
...
def parse_resource(term_id, res_info, name):
  ...
  # 获取 PDF 文档
    ...
    if os.path.exists(os.path.join(BASE_DIR, 'PDFs', file_name + '.pdf')):
      print("------> 文件已经存在！")
      return
    pdfurl = re.search(r'textOrigUrl:"(.*?)"', res).group(1)
    print('------> ' + name + '.pdf')
    if WANT_PDF:
...
if __name__ == '__main__':
  start('http://mooc.study.163.com/course/cloudclass-1000031001', r'F:\Moocs', 1)
