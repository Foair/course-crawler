...
class Resource(object):
    ...
    属性：
        ...
        meta：资源的元信息，比如资源在每个网站的 ID 和 文件名等等；
...
class Playlist(ClassicFile):
    ...
    def write(self, video):
        ...
        self._f.write('%d*title*%s\n\n' % (self._count, '%s %s' % ('.'.join(video.id.split('.')[:-1]), video.name)))
...
class Outline(ClassicFile):
    ...
    def write(self, string, counter, level=2, sign=''):
        ...
        name = '%s%s {%s}%s\n' % ('  ' * level, string, counter[level], sign)
        self._f.write(name)
...
def file_to_save(name):
    name = Resource.regex_file.sub('', name)
    return Resource.regex_spaces.sub(' ', name)
